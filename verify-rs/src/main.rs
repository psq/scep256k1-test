...
use secp256k1::{PublicKey, Secp256k1, SecretKey, Message, Signature, RecoverableSignature};
...
fn main() {
    ...
    let seckey = SecretKey::from_slice(&secp, &secret).expect("32 bytes, within curve order");
    ...
    let serialized_sig = signature.serialize_compact(&secp);
    ...
    let expanded_sig = Signature::from_compact(&secp, &serialized_sig).unwrap();
    ...
    let (rec_id, rec_sig) = recoverable_sig.serialize_compact(&secp);

    let recovered_sig = RecoverableSignature::from_compact(&secp, &rec_sig, rec_id).unwrap();
